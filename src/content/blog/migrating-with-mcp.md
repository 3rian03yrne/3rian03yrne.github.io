...
There are times when even that isn't enough and here's why: All AI models have a knowledge cut off date baked in. This means that your shiny new AI Model contains out of date training knowledge as soon as it comes online.
...
Here is the actual command to add the mcp server at the project scope level, which is the right scope when you want to share config at the repository level.
...
Side by side, the migration itself reads as almost a non-event — the dark, warm design, the copy, the layout all carried straight over. (Some minor differences but all pretty trivial) That was the point: the risk was in the toolchain underneath, not in how the site looks.

Although I ultimately decided to take the design of the site into a new direction, I felt it was important to showcase how simple the initial migration went with just a single MCP connector.
